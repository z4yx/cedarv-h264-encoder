...
{	
	...
	if (mDispHandle <= 0)
	{
		LOGE("open display driver failed, %s", strerror(errno));
		goto ERROR;
	}
	LOGD("open display driver ok");
	...
	if (mLayer == 0)
	{
		LOGE("request layer failed, %s", strerror(errno));
		goto ERROR;
	}
	LOGD("request layer ok");

    mScreenWidth	= ioctl(mDispHandle, DISP_CMD_SCN_GET_WIDTH, args);
    mScreenHeight 	= ioctl(mDispHandle, DISP_CMD_SCN_GET_HEIGHT, args);
	LOGD("get screen width: %d, height: %d", mScreenWidth, mScreenHeight);
	...
	if (error != 0)
	{
		LOGE("get layer information failed, %s", strerror(errno));
		goto ERROR;
	}
	LOGD("get layer information ok");
	...
	LOGD("video : %dx%d, scn: %dx%d", mVideoWidth, mVideoHeight, mScreenWidth, mScreenHeight);

	error = ioctl(mDispHandle, DISP_CMD_LAYER_SET_PARA, &args);
	if (error != 0)
	{
		LOGE("set layer information failed, %s", strerror(errno));
		goto ERROR;
	}

	LOGV("set video layer top");
	args[0] = 0;
	args[1] = mLayer;
	error = ioctl(mDispHandle, DISP_CMD_LAYER_TOP, (void *)args);
	if (error != 0)
	{
		LOGE("set layer top failed, %s", strerror(errno));
		goto ERROR;
	}
...
GET_FRAME_ID:
		...
		if (tmpId != fb_addr.id)
		{
			LOGD("WAIT...., tmpId: %d, fb_addr.id: %d", tmpId, fb_addr.id);
			usleep(1000);
			goto GET_FRAME_ID;
		}
